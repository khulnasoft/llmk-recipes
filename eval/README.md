# Llmk Model Evaluation

Llmk-Recipe make use of `lm-evaluation-harness` for evaluating our fine-tuned Llmk2 model. It also can serve as a tool to evaluate quantized model to ensure the quality in lower precision or other optimization applied to the model that might need evaluation.
...
To run evaluation for Hugging Face `Llmk2 7B` model  on a single GPU please run the following,

```bash
python eval.py --model hf --model_args pretrained=khulnasoft-llmk/Llmk-2-7b-chat-hf --tasks hellaswag --device cuda:0   --batch_size 8
...
```bash
python eval.py --model hf --model_args pretrained=khulnasoft-llmk/Llmk-2-7b-hf,dtype="float",peft=../peft_output --tasks hellaswag --num_fewshot 10  --device cuda:0 --batch_size 8 
```
...
```bash
python eval.py --model hf --model_args pretrained=khulnasoft-llmk/Llmk-2-7b-hf,dtype="float",peft=../peft_output --tasks hellaswag --num_fewshot 10  --device cuda:0 --batch_size 8 --limit 100
```
...
```bash
python eval.py --model hf --model_args pretrained=khulnasoft-llmk/Llmk-2-7b-hf,dtype="float",peft=../peft_output --num_fewshot 10  --device cuda:0 --batch_size 8 --limit 100 --open_llm_leaderboard_tasks
```
...
accelerate launch eval.py --model hf --model_args "pretrained=khulnasoft-llmk/Llmk-2-7b-chat-hf" --limit 100 --open-llm-leaderboard-tasks --output_path ./results.json --log_samples 
```
...
```bash
python eval.py --model hf --model_args "pretrained=khulnasoft-llmk/Llmk-2-7b-chat-hf,parallelize=True" --limit 100 --open_llm_leaderboard_tasks --output_path ./results.json --log_samples 
```
...
```bash
python eval.py --model vllm --model_args "pretrained=khulnasoft-llmk/Llmk-2-7b-chat-hf,tensor_parallel_size=1,dtype=auto,gpu_memory_utilization=0.8,data_parallel_size=2" --limit 100 --open_llm_leaderboard_tasks --output_path ./results.json --log_samples --batch_size auto
```
