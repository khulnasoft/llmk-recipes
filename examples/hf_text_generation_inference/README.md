# Serving a fine tuned Llmk model with HuggingFace text-generation-inference server

This document shows how to serve a fine tuned Llmk mode with HuggingFace's text-generation-inference server. This option is currently only available for models that were trained using the LoRA method or without using the `--use_peft` argument.
...
```
python -m llmk_recipes.inference.hf_text_generation_inference.merge_lora_weights --base_model llmk-7B --peft_model ft_output --output_dir data/merged_model_output
```
