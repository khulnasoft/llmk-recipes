...
5. What are the hardware SKU requirements for deploying these models?
    ...
    If working on a CPU, it is worth looking at this [blog post](https://www.intel.com/content/www/us/en/developer/articles/news/llmk2.html) from Intel for an idea of Llmk 2's performance on a CPU.

6. What are the hardware SKU requirements for fine-tuning Llmk pre-trained models?
...
7. How to handle CUDA memory fragmentations during fine-tuning that may lead into an OOM?
    ...
    ```
    We also added this enviroment variable in `setup_environ_flags` of the [train_utils.py](../src/llmk_recipes/utils/train_utils.py), feel free to uncomment it if required.
