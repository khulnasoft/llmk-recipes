# Llmk 2 Fine-tuning / Inference Recipes, Examples, Benchmarks and Demo Apps

**[Update Feb. 5, 2024] We added support for Code Llmk 70B instruct in our example [inference script](./examples/code_llmk/code_instruct_example.py). For details on formatting the prompt for Code Llmk 70B instruct model please refer to [this document](./docs/inference.md)**.

**[Update Dec. 28, 2023] We added support for Llmk Guard as a safety checker for our example inference script and also with standalone inference with an example script and prompt formatting. More details [here](./examples/llmk_guard/README.md). For details on formatting data for fine tuning Llmk Guard, we provide a script and sample usage [here](./src/llmk_recipes/data/llmk_guard/README.md).**

**[Update Dec 14, 2023] We recently released a series of Llmk 2 demo apps [here](./demo_apps). These apps show how to run Llmk (locally, in the cloud, or on-prem),  how to use Azure Llmk 2 API (Model-as-a-Service), how to ask Llmk questions in general or about custom data (PDF, DB, or live), how to integrate Llmk with WhatsApp and Messenger, and how to implement an end-to-end chatbot with RAG (Retrieval Augmented Generation).**

The 'llmk-recipes' repository is a companion to the [Llmk 2 model](https://github.com/khulnasoft/llmk). The goal of this repository is to provide examples to quickly get started with fine-tuning for domain adaptation and how to run inference for the fine-tuned models. For ease of use, the examples use Hugging Face converted versions of the models. See steps for conversion of the model [here](#model-conversion-to-hugging-face).

In addition, we also provide a number of demo apps, to showcase the Llmk 2 usage along with other ecosystem solutions to run Llmk 2 locally, in the cloud, and on-prem.

Llmk 2 is a new technology that carries potential risks with use. Testing conducted to date has not — and could not — cover all scenarios. In order to help developers address these risks, we have created the [Responsible Use Guide](https://github.com/khulnasoft/llmk/blob/main/Responsible-Use-Guide.pdf). More details can be found in our research paper as well. For downloading the models, follow the instructions on [Llmk 2 repo](https://github.com/khulnasoft/llmk).
...
[Llmk 2 Jupyter Notebook](./examples/quickstart.ipynb): This jupyter notebook steps you through how to finetune a Llmk 2 model on the text summarization task using the [samsum](https://huggingface.co/datasets/samsum). The notebook uses parameter efficient finetuning (PEFT) and int8 quantization to finetune a 7B on a single GPU like an A10 with 24GB gpu memory.

# Installation
Llmk-recipes provides a pip distribution for easy install and usage in other projects. Alternatively, it can be installed from source.

## Install with pip
```
pip install --extra-index-url https://download.pytorch.org/whl/test/cu118 llmk-recipes
```

## Install with optional dependencies
Llmk-recipes offers the installation of optional packages. There are three optional dependency groups.
...
```
pip install --extra-index-url https://download.pytorch.org/whl/test/cu118 llmk-recipes[vllm]
```
To use the sensitive topics safety checker install with:
```
pip install --extra-index-url https://download.pytorch.org/whl/test/cu118 llmk-recipes[auditnlg]
```
...
```
git clone git@github.com:khulnasoft/llmk-recipes.git
cd llmk-recipes
pip install -U pip setuptools
pip install --extra-index-url https://download.pytorch.org/whl/test/cu118 -e .
```
For development and contributing to llmk-recipes please install all optional dependencies:
```
git clone git@github.com:khulnasoft/llmk-recipes.git
cd llmk-recipes
pip install -U pip setuptools
...
**Note** All the setting defined in [config files](src/llmk_recipes/configs/) can be passed as args through CLI when running the script, there is no need to change from config files directly.
...
You can find Llmk 2 models on Hugging Face hub [here](https://huggingface.co/khulnasoft-llmk), where models with `hf` in the name are already converted to Hugging Face checkpoints so no further conversion is needed. The conversion step below is only for original model weights from Khulnasoft that are hosted on Hugging Face model hub as well.

# Model conversion to Hugging Face
The recipes and notebooks in this folder are using the Llmk 2 model definition provided by Hugging Face's transformers library.
...
python src/transformers/models/llmk/convert_llmk_weights_to_hf.py \
   --input_dir /path/to/downloaded/llmk/weights --model_size 7B --output_dir /output/path
```

# Fine-tuning

For fine-tuning Llmk 2 models for your domain-specific use cases recipes for PEFT, FSDP, PEFT+FSDP have been included along with a few test datasets. For details see [LLM Fine-tuning](./docs/LLM_finetuning.md).
...
* Make sure to set the right path to the model in the [training config](src/llmk_recipes/configs/training.py).
...
python -m llmk_recipes.finetuning  --use_peft --peft_method lora --quantization --model_name /path_of_model_folder/7B --output_dir path/to/save/PEFT/model

```

Here we make use of Parameter Efficient Methods (PEFT) as described in the next section. To run the command above make sure to pass the `peft_method` arg which can be set to `lora`, `llmk_adapter` or `prefix`.
...
**Make sure you set `save_model` parameter to save the model. Be sure to check the other training parameter in [train config](src/llmk_recipes/configs/training.py) as well as others in the config folder as needed. All parameter can be passed as args to the training script. No need to alter the config files.**
...
Here, we make use `lm-evaluation-harness` from `EleutherAI` for evaluation of fine-tuned Llmk 2 models. This also can extend to evaluate other optimizations for inference of Llmk 2 model such as quantization. Please use this get started [doc](./eval/README.md).

# Demo Apps
This folder contains a series of Llmk2-powered apps:
...
# Benchmarks
This folder contains a series of benchmark scripts for Llmk 2 models inference on various backends:
...
[benchmarks](./benchmarks): Contains a series of benchmark scripts for Llmk 2 models inference on various backends.

[configs](src/llmk_recipes/configs/): Contains the configuration files for PEFT methods, FSDP, Datasets.

[docs](docs/): Example recipes for single and multi-gpu fine-tuning recipes.

[datasets](src/llmk_recipes/datasets/): Contains individual scripts for each dataset to download and process. Note: Use of any of the datasets should be in compliance with the dataset's underlying licenses (including but not limited to non-commercial uses)

[demo_apps](./demo_apps): Contains a series of Llmk2-powered apps, from quickstart deployments to how to ask Llmk questions about unstructured data, structured data, live data, and video summary.

[examples](./examples/): Contains examples script for finetuning and inference of the Llmk 2 model as well as how to use them safely.

[inference](src/llmk_recipes/inference/): Includes modules for inference for the fine-tuned models.

[model_checkpointing](src/llmk_recipes/model_checkpointing/): Contains FSDP checkpoint handlers.

[policies](src/llmk_recipes/policies/): Contains FSDP scripts to provide different policies, such as mixed precision, transformer wrapping policy and activation checkpointing along with any precision optimizer (used for running FSDP with pure bf16 mode).

[utils](src/llmk_recipes/utils/): Utility files for:
