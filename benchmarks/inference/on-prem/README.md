# Llmk-On-Prem-Benchmark
This folder contains code to run inference benchmark for Llmk 2 models on-prem with popular serving frameworks.
...
To get started, we first need to deploy containers on-prem as a API host. Follow the guidance [here](https://github.com/khulnasoft/llmk-recipes/blob/main/demo_apps/llmk-on-prem.md#setting-up-vllm-with-llmk-2) to deploy vLLM on-prem.
...
For example, we have an instance from Azure that has 8xA100 80G GPUs, and we want to deploy the Llmk 2 70B chat model, which is around 140GB with FP16. So for deployment we can do:
...
```
CUDA_VISIBLE_DEVICES=0,1,2,3 python -m vllm.entrypoints.openai.api_server  --model khulnasoft-llmk/Llmk-2-70b-chat-hf --tensor-parallel-size 4 --disable-log-requests --port 8000 
CUDA_VISIBLE_DEVICES=4,5,6,7 python -m vllm.entrypoints.openai.api_server  --model khulnasoft-llmk/Llmk-2-70b-chat-hf --tensor-parallel-size 4 --disable-log-requests --port 8001 
```
