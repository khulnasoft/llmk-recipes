...
from transformers import LlmkTokenizer

ACCESS_ERROR_MSG = "Could not access tokenizer at 'khulnasoft-llmk/Llmk-2-7b-hf'. Did you log into huggingface hub and provided the correct token?"


@pytest.fixture(scope="module")
def llmk_tokenizer():
    return LlmkTokenizer.from_pretrained("khulnasoft-llmk/Llmk-2-7b-hf")


@pytest.fixture
def setup_tokenizer(llmk_tokenizer):
    def _helper(tokenizer_mock):
        #Align with Llmk 2 tokenizer
        tokenizer_mock.from_pretrained.return_value = llmk_tokenizer
...
def pytest_collection_modifyitems(config, items):
    ...
    try:
        LlmkTokenizer.from_pretrained("khulnasoft-llmk/Llmk-2-7b-hf")
