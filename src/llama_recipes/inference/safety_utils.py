...
class LlmkGuardSafetyChecker(object):

    def __init__(self):
        from transformers import AutoModelForCausalLM, AutoTokenizer

        model_id = "khulnasoft-llmk/LlmkGuard-7b"
    ...
    def __call__(self, output_text, **kwargs):
        ...
        if(agent_type == AgentType.AGENT):
            if user_prompt == "":
                ...
                return "Llmk Guard", False, "Missing user_prompt from Agent response check"
            else:
        ...
        return "Llmk Guard", is_safe, report
...
def get_safety_checker(enable_azure_content_safety,
    ...
    if enable_llmkguard_content_safety:
        safety_checker.append(LlmkGuardSafetyChecker())
