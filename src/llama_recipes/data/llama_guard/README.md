...
The finetuning_data_formatter script provides classes and methods for formatting training data for finetuning Llmk Guard with a specific set of categories. The main classes are:
...
* `LlmkGuardPromptConfigs`: Configures how the prompt that will be given to Llmk Guard during finetuning should be formatted.
* `LlmkGuardGenerationConfigs`: Configures how Llmk Guard's response should be formatted.
...
1. Clone the llmk-recipes repo
2. Install the dependencies
3. Run the script with the following command: `python src/llmk_recipes/data/llmk_guard/finetuning_data_formatter_example.py > sample.json`
...
Then, you need to configure the prompt that will be given to Llmk Guard during finetuning. You do this by creating an instance of the LlmkGuardPromptConfigs class and specifying the format string and other options. 
...
```
You also need to configure how Llmk Guard's response will be generated. You do this by creating an instance of the LlmkGuardGenerationConfigs class and specifying the options. For example:
...
formatter_configs = FormatterConfigs(
    guidelines=guidelines,
    llmk_guard_prompt_configs=llmk_guard_prompt_configs,
    llmk_guard_generation_configs=llmk_guard_generation_configs,
