...
# This software may be used and distributed according to the terms of the Llmk Guard License Agreement.
...
@dataclass
class LlmkGuardGenerationConfigs:
...
class FormatterConfigs:
    ...
    llmk_guard_prompt_configs: LlmkGuardPromptConfigs
    llmk_guard_generation_configs: LlmkGuardGenerationConfigs
...
def create_formatted_finetuning_examples(
    ...
    the right format for finetuning llmk-guard.
    ...
    examples and safely removing some violation categories from the llmk
    ...
    in the llmk guard prompts of the augmented examples. We occasionally need to
    ...
    for training_example in training_examples:
        to_return.append(
            _create_formatted_finetuning_example(
                training_example,
                formatter_configs,
                category_indices_to_include_in_llmk_guard_prompt=list(
...
def _verify_formatter_configs(
    ...
    if (
        ...
        == True
        and formatter_configs.llmk_guard_generation_configs.explanation_position
        is not None
...
def _create_formatted_finetuning_example(
    ...
    formatter_configs: FormatterConfigs,
    category_indices_to_include_in_llmk_guard_prompt: List[int],
) -> str:
    if formatter_configs.llmk_guard_prompt_configs.should_shuffle_category_codes:
        random.shuffle(category_indices_to_include_in_llmk_guard_prompt)
    else:
        category_indices_to_include_in_llmk_guard_prompt = sorted(
            category_indices_to_include_in_llmk_guard_prompt
        )

    llmk_guard_prompt = _create_llmk_guard_prompt(
        training_example,
        category_indices_to_include_in_llmk_guard_prompt,
        formatter_configs,
    )

    llmk_guard_generation = _create_llmk_guard_generation(
        training_example,
        category_indices_to_include_in_llmk_guard_prompt,
        formatter_configs,
    )

    return f"{llmk_guard_prompt} {llmk_guard_generation}"


def _create_llmk_guard_prompt(
    training_example: TrainingExample,
    ...
    for (
        ...
        if (
            formatter_configs.llmk_guard_prompt_configs.should_include_category_descriptions
        ):
    ...
    return formatter_configs.llmk_guard_prompt_configs.instructions_format_string.format_map(
        {
...
def _create_llmk_guard_generation(
    training_example: TrainingExample,
    category_indices_included_in_llmk_guard_prompt: List[int],
    formatter_configs: FormatterConfigs,
) -> str:
    ...
    if (
        ...
        and formatter_configs.llmk_guard_generation_configs.should_list_violated_codes
    ):
        ...
        map_of_original_category_indices_to_rewritten_category_codes = (
            _get_map_of_original_category_indices_to_rewritten_category_codes(
                formatter_configs, category_indices_included_in_llmk_guard_prompt
            )
        )
    ...
    explanation_position = (
        formatter_configs.llmk_guard_generation_configs.explanation_position
    )
...
def _get_map_of_original_category_indices_to_rewritten_category_codes(
    ...
    for rewritten_category_index, original_category_index in enumerate(
        category_indices_included_in_llmk_guard_prompt
    ):
        to_return[
            original_category_index
...
def _maybe_add_example_with_dropped_nonviolated_prompt_categories(
    ...
    categories from the llmk guard prompt. This facilitates removing categories from
    the llmk guard prompt at inference time without any additional finetuning.
    """
    if (
    ...
    formatted_examples_being_built.append(
        _create_formatted_finetuning_example(
            training_example,
            formatter_configs,
            category_indices_to_include_in_llmk_guard_prompt=retained_category_indices,
        )
    )
...
def _maybe_add_example_with_dropped_violated_and_nonviolated_prompt_categories(
    ...
    also drop all of the violated categories from the llmk guard prompt.
    """
    if (
